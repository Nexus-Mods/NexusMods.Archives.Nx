...
public class OutputWriteLock
{
    ...
    // ~3GB/s for NVMe
    // ~32GB/s for RAM
    [Params(80, 250, 3000, 32000)] public int DiskWriteSpeedMB { get; set; }
    ...
    [Params(0.03125F, 0.125F, 0.5F, 1F)] public float BlockSizeMB { get; set; }

    // ~40MB/s ZSTD -16 on modern Ryzen 5XXX. (and close to LZ4 -9)
    // ~22MB/s LZ4 -12 on modern Ryzen 5XXX.
    [Params( /*22, */40)] public int CompressionSpeedMB { get; set; }

    private readonly int _totalMB = 1000;
    private OrderedTaskScheduler _scheduler = null!;

    [IterationSetup]
    public void Setup() => _scheduler = new OrderedTaskScheduler(NumThreads);
    ...
    private readonly byte[] _queueDummy = Array.Empty<byte>();
